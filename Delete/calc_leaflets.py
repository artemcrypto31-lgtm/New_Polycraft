...
# === МАШИНА СОСТОЯНИЙ (Все поля на месте) ===
...
async def smart_edit(message: types.Message, text: str, kb: InlineKeyboardMarkup):
    try:
        ...
        await message.answer(text=text, reply_markup=kb, parse_mode="HTML", disable_notification=True)
# ==========================================
# 1. ФОРМАТ
# ==========================================
...
async def info_l_format(callback: types.CallbackQuery):
    await callback.answer()
    text = (
...
async def render_step_2(message: types.Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="2️⃣ Двухсторонняя (4+4)", callback_data="l_col_4+4")],
...
async def info_l_color(callback: types.CallbackQuery):
    await callback.answer()
    text = (
...
async def info_l_proc(callback: types.CallbackQuery):
    await callback.answer()
    text = (
...
async def info_l_ptype(callback: types.CallbackQuery):
    await callback.answer()
    text = (
...
async def render_step_5(message: types.Message, state: FSMContext):
    ...
    if p_type in ["Мелованная", "Матовая"]:
        weights = ["80", "90", "115", "130", "150", "170", "200", "250", "300", "350"]
...
async def info_l_circ(callback: types.CallbackQuery):
    await callback.answer()
    text = (
...
async def manual_circulation(message: types.Message, state: FSMContext, bot: Bot):
    if not message.text.isdigit() or int(message.text) < 100:
        await message.answer("⚠️ Пожалуйста, введите число не меньше 100.", disable_notification=True)
        return
    await state.update_data(count=message.text)
    data = await state.get_data()
    try: await message.delete()
    except: pass
    await show_final_summary(message, state, bot, edit_id=data.get('last_msg_id'))

@router.callback_query(CalcLeaflets.step_circulation, F.data.startswith("l_cnt_"))
async def btn_circulation(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    await callback.answer()
    await state.update_data(count=callback.data.replace("l_cnt_", ""))
    await show_final_summary(callback.message, state, bot)

async def show_final_summary(message: types.Message, state: FSMContext, bot: Bot, edit_id: int = None):
    data = await state.get_data()
    
    # Собираем красивый список параметров (жирным)
    params = (
    ...
    await state.update_data(final_summary=params)
    user_id = message.chat.id
    profile = mock_db.USERS_DB.get(user_id)
    
    # Проверяем, заполнены ли у пользователя ВСЕ ключевые данные (включая email)
    if profile and profile.get('phone') and profile.get('email'):
        text = (
        ...
        # ВАЖНО: Сохраняем категорию и ТЗ, чтобы регистрация знала, что отправить в конце
        await state.update_data(order_category="листовки", final_summary=params)
        ...
        text = (
            ...
            "Чтобы менеджер мог рассчитать стоимость и связаться с вами, нужно пройти быструю регистрацию (всего несколько вопросов).\n\n"
            "<i>Это создаст ваш Личный кабинет, где вы сможете отслеживать статус этого и будущих заказов.</i>"
        )
        kb = InlineKeyboardMarkup(inline_keyboard=[
            # Заменили l_reg_start на единый start_common_reg
            [InlineKeyboardButton(text="📝 ПРЕДСТАВИТЬСЯ И ОТПРАВИТЬ", callback_data="start_common_reg")],
    ...
    if edit_id:
        ...
        except: await message.answer(text, reply_markup=kb, parse_mode="HTML", disable_notification=True)
    else:
        await smart_edit(message, text, kb)

@router.callback_query(F.data == "l_submit")
async def final_submit(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    await callback.answer()
    data = await state.get_data()
    order_id = mock_db.add_order(callback.from_user.id, data['final_summary'], category="листовки")
    
    # Отправка уведомления админу (остается как была)
    admin_id = os.getenv("ADMIN_ID")
    if admin_id:
        try: await bot.send_message(admin_id, f"⚡️ <b>ЗАКАЗ #{order_id} (Листовки)</b>\n👤 @{callback.from_user.username}\n\n{data['final_summary']}", parse_mode="HTML")
        except: pass

    # Финальный текст и 3 кнопки навигации
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🏗 В каталог товаров", callback_data="menu_catalog")],
            [InlineKeyboardButton(text="🏠 Главное меню", callback_data="back_to_main")]
        ]),
        parse_mode="HTML"
    )
    await state.clear()
    await message.answer("✅ Контакты сохранены! Отправляем заказ...", disable_notification=True)
    await final_submit(types.CallbackQuery(id="0", from_user=message.from_user, chat_instance="0", message=message, data="l_submit"), state, bot)