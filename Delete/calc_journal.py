# === START handlers/calc_journal.py ===
from aiogram import Router, F, types, Bot
from aiogram.fsm.context import FSMContext
from aiogram.fsm.state import State, StatesGroup
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
from handlers.user_catalog import kb_cat_multipage, TEXT_MULTIPAGE
import os
import mock_db
...
@router.callback_query(F.data == "start_calc_journal")
...
async def step_finish_summary(callback: types.CallbackQuery, state: FSMContext):
    await callback.answer()
    ...
    if not profile or not profile.get('name') or not profile.get('phone'):
...
async def reg_address(message: types.Message, state: FSMContext, bot: Bot):
    data = await state.get_data()
    # Сохраняем в базу
    mock_db.update_user(
        message.chat.id, 
    ...
    await finalize_order(message.chat, state, bot, message)
...
async def submit_order_handler(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    await callback.answer()
    try: await callback.message.delete()
    except: pass
    await finalize_order(callback.message.chat, state, bot, callback.message)

async def finalize_order(user_obj, state: FSMContext, bot: Bot, message_obj):
    data = await state.get_data()
    summary = data.get('final_summary', 'Нет данных')
    
    # 1. Сохраняем заказ в базу
    order_id = "UNKNOWN"
    if hasattr(mock_db, 'add_order'):
        order_id = mock_db.add_order(user_obj.id, summary, category="журналы")
    
    # 2. Уведомляем админов
    admin_ids = [x.strip() for x in os.getenv("ADMIN_ID", "").split(",") if x.strip()]
    
    db_user = mock_db.get_user(user_obj.id)
    client_name = db_user.get('name', user_obj.first_name) if db_user else user_obj.first_name
    client_phone = db_user.get('phone', '-') if db_user else "-"
